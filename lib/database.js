...
async function natsPush(token) {
    ...
    nc.publish(device_id, sc.encode(JSON.stringify({resources, name})));
...
async function saveTempAuth(state, accessToken, locationId) {
  try {
    ...
    await container.items.upsert({
        ...
        accessToken,
        locationId,
        ttl: 300            // Automatically deletes after 5 minutes (security!)
    });
  } catch(err) {
    console.log(err)
  }
...
async function getTempAuth(state) {
  const container = await initializeCosmosContainer();
  const { resource: authData } = await container.item(state, state).read();
  return authData
}



module.exports = { getTempAuth, saveTempAuth, addDrinks, sendTokens, createBusinessName, getReports, getPurchases, natsPurchases, pushOffer, addItem, natsPush, getData, updateItem, deleteItem, natsGet, getOffers, getName, getDrinks, getUser, finishTutorial, saveCode }
