...
async function sendSecretAuth(secret, token) {
  try {
    const { nc, sc } = await initializeNATS();
    const { device_id } = await getUser(token);
  ...
  nc.publish(
    subject,
    sc.encode(JSON.stringify({ secret }))
  );
  return 'success'
  } catch(err) {
    console.log(err)
...
module.exports = { sendSecretAuth, getTempAuth, saveTempAuth, addDrinks, sendTokens, createBusinessName, getReports, getPurchases, natsPurchases, pushOffer, addItem, natsPush, getData, updateItem, deleteItem, natsGet, getOffers, getName, getDrinks, getUser, finishTutorial, saveCode }
