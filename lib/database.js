...
async function getData(token) {
    ...
    const hydratedProducts = products.map(product => {
        ...
        return {
          ...
          fileUrl: product.fileUrl || "/images/placeholder.webp",
...
async function inputPurchases(rows, token) {
  ...
  for (const row of rows) {
    ...
    // Ensure price is treated as a precise float
    ...
    if (!grouped[orderId]) {
      grouped[orderId] = {
        ...
        orderId,
        email: row.email,
        timestamp: dateObj.toISOString(), // Standardizing to ISO string format
        items: [],
        totalPrice: 0,
        business: business,
        type: "report",
    ...
    grouped[orderId].items.push({
      ...
      price: itemPrice, // Properly parsed float
    });

    // Accumulate total using the parsed float
    grouped[orderId].totalPrice += itemPrice;
  }
...
async function saveCustomizationsLibrary(incomingModifiers, incomingRules, userData) {
  ...
  // =========================================================================
  // ENFORCE ABSOLUTE ID UNIQUENESS FOR EACH INDIVIDUAL SAVE RECORD
  // Generates an ID like: modifiers_lib_TestBusiness_1717056000000
  // =========================================================================
  const timestamp = Date.now();
  const libraryId = `modifiers_lib_${businessId}_${timestamp}`;
  
  console.log(`🔒 Saving UNIQUE multi-tenant configuration record. ID: ${libraryId} | Partition Key: ${businessId}`);

  // Deduplication Engine: Keep modifier items inside arrays strictly unique by name
  ...
  // =========================================================================
  // CREATE FRESH RECORD ONLY (Removes replace code pathway)
  // Because the ID appends a timestamp, this entry is fundamentally unique.
  // =========================================================================
  const newLibDoc = {
    id: libraryId,                         // Globally unique document target pointer
    type: "modifier_library",
    business: businessId,                  // Partition Key remains tied strictly to the vendor entity ID
    created_at: new Date(timestamp).toISOString(),
    modifiers: cleanModifiers,
    customizationRules: incomingRules || defaultRules 
  };
  
  // Create an explicit new document row slot within Cosmos DB
  const result = await cosmosContainer.items.create(newLibDoc, { partitionKey: businessId });
  return result.resource;
}
